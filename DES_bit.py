...
# split_list(a, n)                  -> Bagi list jadi potongan ukuran n
# xor(t1, t2)                       -> XOR antara dua list bit
# shift_left(lst, n)                -> Geser list bit ke kiri (circular)
...
# pad_text(text)                    -> Tambah padding (PKCS5)
...
def permute(block, table, name="PERMUTE"):
    ...
    logging.debug(f"[{name}] Input: {block} -> Output: {result}")
    return result
...
def xor(t1, t2):
    ...
    logging.debug(f"[XOR] {t1} XOR {t2} -> {result}")
    return result

def shift_left(lst, n):
    result = lst[n:] + lst[:n]
    logging.debug(f"[SHIFT_LEFT {n}] {lst} -> {result}")
    return result
...
def generate_keys(key_bits):
    ...
    logging.info(f"[KEY] Original bits after PC1 split: C0={C}, D0={D}")
    ...
    for i, shift_val in enumerate(SHIFT_SCHEDULE):
        ...
        keys.append(subkey)
        logging.info(f"[SUBKEY {i+1}] {subkey}")
...
def feistel(right, subkey, round_num=None):
    ...
    logging.info(f"[ROUND {round_num}] Right expanded: {expanded}")
    
    temp = xor(expanded, subkey)
    logging.info(f"[ROUND {round_num}] After XOR with subkey: {temp}")
    
    substituted = substitute(temp)
    logging.info(f"[ROUND {round_num}] After S-Boxes: {substituted}")
    
    permuted = permute(substituted, P, name="P-PERM")
    logging.info(f"[ROUND {round_num}] After P-Permutation: {permuted}")
    return permuted

# ----------------------------- DES Core -----------------------------
def des_rounds(block, keys, encrypt=True):
    block = permute(block, IP, name="IP")
    logging.info(f"[IP] After Initial Permutation: {block}")
    ...
    for i in round_order:
        ...
        left = temp
        logging.info(f"[ROUND {i+1}] L={left}, R={right}")

    combined = right + left
    final_block = permute(combined, FP, name="FP")
    logging.info(f"[FP] After Final Permutation: {final_block}")
    return final_block
...
def encrypt(plaintext: str, key: str):
    ...
    logging.info(f"[KEY] Original key bits: {key_bits}")
...
if __name__ == "__main__":
    ...
    logging.info(f"Cipher bits (len={len(cipher_bits)}): {cipher_bits}")
